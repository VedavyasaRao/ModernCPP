...
// a function consuming a shared_ptr can take it by value or by rvalue reference
shared_ptr<D> pass_through(shared_ptr<D> p)
{
    p->bar();
    return p;
}
...
int main()
{
    cout << "1) Unique ownership semantics demo" << endl;
    {
        ...
        shared_ptr<D> p = make_unique<D>();
 
        // Transfer ownership to `pass_through`,
        // which in turn transfers ownership back through the return value
        shared_ptr<D> q = pass_through(move(p));
    ...
    {
        using unique_file_t = shared_ptr<FILE>;
    ...
    cout  << endl << "5) Array form of shared_ptr demo" << endl;
    {
        shared_ptr<D[]> p(new D[3]);
    ...
    *p = 20;
    ...
    cout << endl << "8) unique demo" << endl;    
    foo = shared_ptr<int>(new int(30));
    cout << boolalpha << "unique\t\t: " << foo.unique() << endl;
    cout << boolalpha << "use_count\t: " << foo.use_count() << endl;
    ...
    //foo:null
    shared_ptr<int[]> foo2(new int [10],default_delete<int[]>());
    //foo:(int[10])
